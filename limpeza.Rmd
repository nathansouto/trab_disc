...
```{r}
colnames(dados) <- c("idade", "atrito", "viagemTrabalho", 
                     ...
                     "taxaMensal", "nEmpresasTrabalhadas", "horaExtra", 
...
```{r}
dados %>%
  group_by(sexo, atrito) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n)) %>%
  ggplot(aes(x=sexo, fill=atrito)) +
...
```{r}
dados %>%
  group_by(cargo, atrito) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n)) %>%
  ggplot(aes(x=atrito, fill=cargo)) +
  geom_bar(size=3, position="dodge", aes(y = freq), stat = 'identity') +
  xlab("Atrito") +                             
  ylab("Frequência relativa") +                                  
...
```{r}
dados %>%
ggplot(aes(x=idade, y=renda, col=atrito)) +
  geom_point(size=3) +
  xlab("Idade") +                             
  ylab("Salário") +                                  
...
```{r}
dados %>%
ggplot(aes(x=distanciaCasa, y=renda, col=atrito)) +
  geom_point(size=2) +
  xlab("Distancia casa") +                             
  ylab("Salário") +                                  
  ggtitle("Frequência das respostas de atrito por distancia e renda")
```
...
```{r}
dados$atrito = factor(dados$atrito, levels = c('Sim','Nao'))
...
fit = rpart(formula = atrito ~ ., method = 'class', data = train, parms = list(split = 'information'))
...
```{r}
rpart.plot(fit, type=0, extra=101, box.palette="gray", branch.lty = 3, nn=TRUE, cex=1)
```